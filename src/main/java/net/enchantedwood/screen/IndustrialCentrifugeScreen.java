        ...
        if (this.focusedSlot != null && !this.focusedSlot.hasStack() && this.focusedSlot.id < 4) {
            switch (this.focusedSlot.id) {
                case 0 -> context.drawTooltip(this.textRenderer, List.of(
                        Text.literal("§e📥 Raw Botanical / Mineral Input"),
                        Text.literal("§7Insert unrefined organic or mineral matter:"),
                        Text.literal("§f• Slime Balls, Magma Cream, Crimson Fungus, Kelp,"),
                        Text.literal("§f• Dragon Fruit, Wasabi Root, Glow Berries, etc.")
                ), mouseX, mouseY);
                case 1 -> context.drawTooltip(this.textRenderer, List.of(
                        Text.literal("§a✨ Purified Chemical Essence Output"),
                        Text.literal("§7Refined active medical extract appears here:"),
                        Text.literal("§f• Alkaline Base, Cryo-Thermal, Oxygenated, Nanite, Adrenal")
                ), mouseX, mouseY);
                case 2 -> context.drawTooltip(this.textRenderer, List.of(
                        Text.literal("§6🛢️ Chemical / Mineral Byproduct Output"),
                        Text.literal("§7Secondary separated matter appears here:"),
                        Text.literal("§f• Sulfur Dust, Volcanic Ash, Bone Meal, Sugar, Glowstone")
                ), mouseX, mouseY);
                case 3 -> context.drawTooltip(this.textRenderer, List.of(
                        ...
                        Text.literal("§f• Dramatically accelerates centrifugal separation speed.")
                ), mouseX, mouseY);
            }
        }
    }
