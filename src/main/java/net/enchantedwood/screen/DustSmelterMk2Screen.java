...
public class DustSmelterMk2Screen extends HandledScreen<DustSmelterMk2ScreenHandler> {
    private static final Identifier GUI_TEXTURE = Identifier.of(EnchantedWoodMod.MOD_ID, "textures/gui/container/dust_smelter_mk2_gui.png");

    public DustSmelterMk2Screen(DustSmelterMk2ScreenHandler handler, PlayerInventory inventory, Text title) {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (mouseX >= x + 17 && mouseX <= x + 35 && mouseY >= y + 19 && mouseY <= y + 71) {
            context.drawTooltip(this.textRenderer, List.of(
                    ...
                    Text.literal("§7Usage: 75 FE/t"),
                    Text.literal("§aDual-Chamber Parallel Smelting")
            ), mouseX, mouseY);
        }
        ...
        if (this.focusedSlot != null && !this.focusedSlot.hasStack() && this.focusedSlot.id < 5) {
            switch (this.focusedSlot.id) {
                case 0 -> context.drawTooltip(this.textRenderer, List.of(
                        Text.literal("§e📥 Dust Input Chamber A"),
                        Text.literal("§7Insert crushed metal dusts or raw ores:"),
                        Text.literal("§f• Iron, Copper, Gold, Titanium, Aluminum, etc.")
                ), mouseX, mouseY);
                case 1 -> context.drawTooltip(this.textRenderer, List.of(
                        Text.literal("§e📥 Dust Input Chamber B"),
                        Text.literal("§7Insert secondary metal dusts for parallel processing:"),
                        Text.literal("§f• Smelts concurrently with Chamber A!")
                ), mouseX, mouseY);
                case 2 -> context.drawTooltip(this.textRenderer, List.of(
                        Text.literal("§a✨ Ingot Output A"),
                        Text.literal("§7Smelted metal ingots from Chamber A appear here.")
                ), mouseX, mouseY);
                case 3 -> context.drawTooltip(this.textRenderer, List.of(
                        Text.literal("§a✨ Ingot Output B"),
                        Text.literal("§7Smelted metal ingots from Chamber B appear here.")
                ), mouseX, mouseY);
                case 4 -> context.drawTooltip(this.textRenderer, List.of(
                        ...
                        Text.literal("§f• Drastically accelerates smelting rate down to 4 ticks!")
                ), mouseX, mouseY);
            }
        }
    }
