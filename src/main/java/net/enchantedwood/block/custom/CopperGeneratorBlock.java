...
public class CopperGeneratorBlock extends BlockWithEntity {
    public static final MapCodec<CopperGeneratorBlock> CODEC = createCodec(CopperGeneratorBlock::new);
    public static final EnumProperty<Direction> FACING = Properties.HORIZONTAL_FACING;
    public static final BooleanProperty LIT = Properties.LIT;

    public CopperGeneratorBlock(Settings settings) {
        super(settings);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world instanceof ServerWorld serverWorld && type == ModBlockEntities.COPPER_GENERATOR_BLOCK_ENTITY) {
            return (w, pos, st, blockEntity) -> CopperGeneratorBlockEntity.tick(serverWorld, pos, st, (CopperGeneratorBlockEntity) blockEntity);
        }
        return null;
    }
    ...
    protected void onStateReplaced(BlockState state, ServerWorld world, BlockPos pos, boolean moved) {
        if (!state.isOf(world.getBlockState(pos).getBlock())) {
            ...
            if (blockEntity instanceof CopperGeneratorBlockEntity generator) {
