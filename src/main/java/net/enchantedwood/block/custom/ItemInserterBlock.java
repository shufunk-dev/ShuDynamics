...
public class ItemInserterBlock extends BlockWithEntity implements Wrenchable {
    public static final MapCodec<ItemInserterBlock> CODEC = createCodec(ItemInserterBlock::new);
    ...
    public ItemInserterBlock(Settings settings) {
        super(settings);
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new ItemInserterBlockEntity(pos, state);
    }

    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world instanceof ServerWorld serverWorld && type == ModBlockEntities.ITEM_INSERTER_BLOCK_ENTITY) {
            return (w, pos, st, blockEntity) -> ItemInserterBlockEntity.tick(serverWorld, pos, st, (ItemInserterBlockEntity) blockEntity);
        }
        return null;
    }
    ...
    public PipeSide getPipeSide(BlockView world, BlockPos pos, Direction side, Direction facing) {
        ...
        if (selfBe instanceof ItemInserterBlockEntity inserter && inserter.isDisconnected(side)) {
    ...
    public ActionResult onWrenched(World world, BlockPos pos, PlayerEntity player, Direction side) {
        ...
        if (be instanceof ItemInserterBlockEntity inserter) {
            boolean disconnected = inserter.toggleConnection(side);
    ...
    public ActionResult onShiftWrenched(World world, BlockPos pos, PlayerEntity player, Direction side) {
        if (!world.isClient()) {
            ...
            player.sendMessage(Text.literal("§6[Wrench] §eDismantled Item Inserter"), true);
        }
        return ActionResult.SUCCESS;
    }
    ...
    public BlockState onBreak(World world, BlockPos pos, BlockState state, net.minecraft.entity.player.PlayerEntity player) {
        ...
        if (blockEntity instanceof ItemInserterBlockEntity inserter) {
            ItemScatterer.spawn(world, pos, inserter.getItems());
        }
