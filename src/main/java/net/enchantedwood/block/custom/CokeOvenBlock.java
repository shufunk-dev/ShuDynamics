...
public class CokeOvenBlock extends BlockWithEntity {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world instanceof ServerWorld serverWorld && type == ModBlockEntities.COKE_OVEN_BLOCK_ENTITY) {
            return (w, pos, st, blockEntity) -> CokeOvenBlockEntity.tick(serverWorld, pos, st, (CokeOvenBlockEntity) blockEntity);
        }
        return null;
    }
    ...
    protected void onStateReplaced(BlockState state, ServerWorld world, BlockPos pos, boolean moved) {
        if (!state.isOf(world.getBlockState(pos).getBlock())) {
            ...
            if (blockEntity instanceof CokeOvenBlockEntity cokeOven) {
                ItemScatterer.spawn(world, pos, cokeOven);
            }
