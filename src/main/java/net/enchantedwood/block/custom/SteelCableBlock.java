...
import net.enchantedwood.block.entity.SteelCableBlockEntity;
import net.enchantedwood.energy.EnergyProvider;
import org.jetbrains.annotations.Nullable;

public class SteelCableBlock extends BlockWithEntity {
    public static final MapCodec<SteelCableBlock> CODEC = createCodec(SteelCableBlock::new);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new SteelCableBlockEntity(pos, state);
    }

    @Nullable
    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world instanceof ServerWorld serverWorld && type == ModBlockEntities.STEEL_CABLE_BLOCK_ENTITY) {
            return (w, pos, st, blockEntity) -> SteelCableBlockEntity.tick(serverWorld, pos, st, (SteelCableBlockEntity) blockEntity);
        }
        return null;
    }
    ...
    public boolean canConnectTo(BlockView world, BlockPos pos, Direction direction) {
        ...
        if (neighborState.getBlock() instanceof SteelCableBlock ||
