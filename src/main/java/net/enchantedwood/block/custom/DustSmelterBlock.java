...
public class DustSmelterBlock extends HorizontalFacingBlock implements BlockEntityProvider {
    public static final MapCodec<DustSmelterBlock> CODEC = createCodec(DustSmelterBlock::new);
    public static final BooleanProperty LIT = Properties.LIT;
    public static final EnumProperty<GearTier> GEAR_TIER = EnumProperty.of("gear_tier", GearTier.class);

    public DustSmelterBlock(Settings settings) {
        super(settings);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world instanceof ServerWorld serverWorld && type == ModBlockEntities.DUST_SMELTER_BLOCK_ENTITY) {
                ...
                if (handStack.getItem() instanceof GearItem gearItem) {
                    ...
                    if (newTier.ordinal() > currentTier.ordinal()) {
                        ...
                        if (hasRedstone || player.isCreative()) {
                            ...
                            dustSmelterEntity.setStack(2, new ItemStack(gearItem));
    ...
    protected void onStateReplaced(BlockState state, ServerWorld world, BlockPos pos, boolean moved) {
        if (!state.isOf(world.getBlockState(pos).getBlock())) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof DustSmelterBlockEntity dustSmelterEntity) {
                ItemScatterer.spawn(world, pos, dustSmelterEntity);
            }
