...
public class AluminumBatteryBlock extends BlockWithEntity {
    public static final MapCodec<AluminumBatteryBlock> CODEC = createCodec(AluminumBatteryBlock::new);
    public static final EnumProperty<Direction> FACING = Properties.HORIZONTAL_FACING;

    public AluminumBatteryBlock(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new AluminumBatteryBlockEntity(pos, state);
    }

    @Nullable
    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world instanceof ServerWorld serverWorld && type == ModBlockEntities.ALUMINUM_BATTERY_BLOCK_ENTITY) {
            return (w, pos, st, blockEntity) -> AluminumBatteryBlockEntity.tick(serverWorld, pos, st, (AluminumBatteryBlockEntity) blockEntity);
        }
        return null;
    }
    ...
    protected void onStateReplaced(BlockState state, ServerWorld world, BlockPos pos, boolean moved) {
        if (!state.isOf(world.getBlockState(pos).getBlock())) {
            ...
            if (blockEntity instanceof AluminumBatteryBlockEntity battery) {
