...
public class HyposprayItem extends Item {
            ...
            if (stack.isOf(ModItems.HYPOSPRAY) && entity instanceof LivingEntity livingEntity) {
                ...
                if (!isLoaded(stack)) {
                    ...
                    if (!offhand.isEmpty() && offhand.getItem() instanceof HyposprayCartridgeItem cartridgeItem) {
                        if (!world.isClient()) {
                            setLoadedCartridge(stack, cartridgeItem.getCartridgeType());
                            offhand.decrement(1);
                            ...
                            player.sendMessage(Text.literal("§e✦ Loaded Hypospray with " + cartridgeItem.getCartridgeType().title + " ✦"), true);
                        }
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ...
        if (user.isSneaking() && isLoaded(hyposprayStack)) {
            if (!world.isClient()) {
                ...
                if (loadedType != null) {
                    user.giveItemStack(new ItemStack(getCartridgeItem(loadedType)));
                }
        ...
        if (!isLoaded(hyposprayStack)) {
            ...
            if (!offhand.isEmpty() && offhand.getItem() instanceof HyposprayCartridgeItem cartridgeItem) {
                if (!world.isClient()) {
                    setLoadedCartridge(hyposprayStack, cartridgeItem.getCartridgeType());
                    offhand.decrement(1);
                    ...
                    user.sendMessage(Text.literal("§e✦ Loaded Hypospray with " + cartridgeItem.getCartridgeType().title + " ✦"), true);
                }
                user.swingHand(hand);
                return ActionResult.SUCCESS;
            } else {
    ...
    public ActionResult executeInoculation(World world, PlayerEntity user, LivingEntity target, ItemStack hyposprayStack, Hand hand) {
        ...
        if (!world.isClient() && world instanceof ServerWorld serverWorld) {
            ...
            if (isOverdose) {
                ...
                applyEffects(loadedType, target, true);
            } else {
                ...
                applyEffects(loadedType, target, false);
    ...
    public static void applyEffects(HyposprayCartridgeItem.Type type, LivingEntity target, boolean diminished) {
        ...
        switch (type) {
            case ACID_NEUTRALIZING -> {
                int duration = (int) (20 * 60 * 6 * factor);
            ...
            case NANITE_TRAUMA -> {
                target.heal(diminished ? 4.0f : 8.0f);
                int duration = (int) (20 * 20 * factor);
            ...
            case ADRENALINE_STIM -> {
                int duration = (int) (20 * 60 * 3 * factor);
    ...
    public Text getName(ItemStack stack) {
        if (isLoaded(stack)) {
            ...
            if (type != null) {
                return Text.literal("Hypospray [" + type.title.replaceAll("§[0-9a-fk-or]", "") + "]");
            }
        }
    ...
    public void appendTooltip(ItemStack stack, Item.TooltipContext context, TooltipDisplayComponent displayComponent, Consumer<Text> textConsumer, TooltipType type) {
        ...
        if (isLoaded(stack)) {
            ...
            if (loadedType != null) {
                textConsumer.accept(Text.literal("§a✦ Chamber: §f" + loadedType.title + " §7(Ready to fire)"));
